"""Run promotion-aware MR approval calibration across a focused parameter grid."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run MR approval calibration across a focused parameter grid.")
    ...
    parser.add_argument("--mr-sigma-entries", nargs="+", type=float, default=[1.2, 1.3, 1.4])
    parser.add_argument("--mr-reclaim-modes", nargs="+", default=["off", "soft"])
    parser.add_argument("--mr-cooldown-bars", nargs="+", type=int, default=[0, 1])
    parser.add_argument("--mr-first-outside-modes", nargs="+", choices=("on", "off"), default=["off"])
    ...
    parser.add_argument("--engine-mode", choices=("mr", "both"), default="mr")
    parser.add_argument("--allocator-policy", choices=("none", "v1", "v2", "open_proxy_v1"), default="none")
    ...
    print(
        f"MR approval calibration outputs written to {summary['output_dir']}\n"
        f"Best candidate: {best.get('label')} | approval={best.get('approval_rate', 0):.2%} | "
